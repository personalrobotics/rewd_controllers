...
//=============================================================================
bool JointGroupPositionController::init(hardware_interface::RobotHW *robot,
                                        ros::NodeHandle &n) {
...
//=============================================================================
void JointGroupPositionController::update(const ros::Time &time,
...
void JointGroupPositionController::setCommand(
  ...
  if (msg.name.size() != numControlledDofs) {
    ROS_ERROR_STREAM("Received command with incorrect number of names:"
                     << " expected " << numControlledDofs << ", got "
                     << msg.name.size());
    return;
  }
  ...
  for (size_t i = 0; i < numControlledDofs; ++i) {
    ...
    if (dofIndex == dart::dynamics::INVALID_INDEX) {
      ROS_ERROR_STREAM("There is DegreeOfFreedom named '"
                       << dof->getName() << "' under control.");
      return;
    }
...
//=============================================================================
PLUGINLIB_EXPORT_CLASS(rewd_controllers::JointGroupPositionController,
